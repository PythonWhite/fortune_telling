...
from ext.web.text import text
# 字体的位置，不同版本的系统会有不同
font_path = './ext/web/simsun.ttc'
# 生成几位数的验证码
number = 8
# 生成验证码图片的高度和宽度
size = (201, 30)
...
class CaptchaHandler(BaseHandler):
    ...
    def get(self):
        ...
        flip, image = gene_code(theme)
        flip = ','.join([str(c) for c in flip])
        self.r.setex('captcha-%s' % t, flip, 100)
